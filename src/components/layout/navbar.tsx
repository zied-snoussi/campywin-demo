...
export function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm shadow-sm border-b border-gray-200/60 dark:border-gray-800/60">
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <Link href="/" className="flex items-center gap-2 flex-shrink-0 hover:opacity-80 transition-opacity">
            <Image
              ...
              alt="CampyWin" width={36} height={36}
              className="object-contain" style={{ width: 36, height: 'auto' }}
            />
          ...
          <div className="hidden lg:flex items-center space-x-6 xl:space-x-8">
            {NAV_LINKS.map((l) => {
              ...
              return (
                ...
                >
                  {l.label}
                  <span className={`absolute bottom-0 left-0 h-0.5 bg-emerald-600 dark:bg-emerald-400 transition-all duration-300 ${active ? 'w-full' : 'w-0 group-hover:w-full'}`} />
                </Link>
              );
            })}
          </div>
            ...
            {user ? (
              <>
                <button onClick={() => router.push(dashPath)}
                  className="hidden sm:flex items-center gap-1.5 px-3 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded-md transition-all shadow-sm hover:shadow-md"
                >
      ...
      {/* Mobile dropdown */}
      {mobileOpen && (
        <div className="lg:hidden bg-white dark:bg-gray-900 shadow-lg border-t border-gray-200 dark:border-gray-800">
          ...
          <div className="pt-3 pb-4 border-t border-gray-200 dark:border-gray-700 px-4 space-y-2">
            {user ? (
              <>
                <button onClick={() => { router.push(dashPath); setMobileOpen(false); }}
                  className="w-full px-4 py-2.5 text-center rounded-xl text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700">
                  Dashboard
                </button>
                <button onClick={() => { logout(); router.push('/'); setMobileOpen(false); }}
                  className="w-full px-4 py-2.5 text-center rounded-xl text-sm font-medium text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 border border-red-200 dark:border-red-800">
                  Sign Out
                </button>
              </>
            ) : (
              <>
                <button onClick={() => { router.push('/login'); setMobileOpen(false); }}
                  className="w-full px-4 py-2.5 text-center rounded-xl text-sm font-medium text-gray-700 dark:text-gray-200 border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">
                  Sign In
                </button>
                <button onClick={() => { router.push('/login'); setMobileOpen(false); }}
                  className="w-full px-4 py-2.5 text-center rounded-xl text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700">
