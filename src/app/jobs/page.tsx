  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">{filtered.length} positions available</p>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          {filtered.map(job => (
            <div
              key={job.id}
              className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm hover:shadow-lg transition-all border border-gray-100 dark:border-gray-700 cursor-pointer group"
              onClick={() => setSelected(job)}
            >
              <div className="flex items-start justify-between mb-3">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center text-white font-bold text-lg shadow-md">
                  {job.company[0]}
                </div>
                {applied.includes(job.id) && (
                  <Badge className="bg-emerald-100 text-emerald-700 border-emerald-300"><CheckCircle className="w-3 h-3 mr-1 inline" />Applied</Badge>
                )}
              </div>
              <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-1 group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">{job.title}</h3>
              <p className="text-sm font-medium text-gray-600 dark:text-gray-300 mb-3">{job.company}</p>
              <div className="flex flex-wrap gap-3 text-xs text-gray-500 dark:text-gray-400 mb-4">
                ...
                <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5" /> {new Date(job.postedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
              </div>
              <div className="flex flex-wrap gap-2 mb-4">
                {job.tags.map(t => (
                  <span key={t} className="text-xs px-2 py-1 bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400 rounded-full">{t}</span>
                ))}
              </div>
            ...
            <>
              <DialogHeader className="mb-4">
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center text-white font-bold text-2xl shadow-md">
                    {selected.company[0]}
                  </div>
                  <div>
              ...
              <div className="flex flex-wrap gap-3 text-xs text-gray-500 mb-5">
                <span className="flex items-center gap-1 bg-gray-50 dark:bg-gray-700 px-3 py-1.5 rounded-full"><MapPin className="w-3.5 h-3.5" /> {selected.location}</span>
                <span className="flex items-center gap-1 bg-gray-50 dark:bg-gray-700 px-3 py-1.5 rounded-full"><Briefcase className="w-3.5 h-3.5" /> {selected.type}</span>
                <span className="flex items-center gap-1 bg-gray-50 dark:bg-gray-700 px-3 py-1.5 rounded-full"><Users className="w-3.5 h-3.5" /> {selected.applicants} applicants</span>
              </div>

              <div className="mb-5">
                <p className="font-semibold text-gray-900 dark:text-white mb-2">About the Role</p>
              ...
              <div className="flex items-center justify-between pt-4 border-t">
                ...
                {applied.includes(selected.id) ? (
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (!user) { jobRouter.push('/login'); return; }
